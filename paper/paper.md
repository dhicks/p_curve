...
@Fig:evidence_severity shows the results of the severity analysis as p-values.  By the weak severity criterion, when the results of the severity analysis are $> .05$, the test output does not provide evidence against the null hypothesis and supporting the target hypothesis.  *Using the severity conception of evidence, a slope of one does not provide evidence to support the zero effect hypothesis and non-linearity does not provide evidence to support the mixed-effect hypothesis.* 
...
The mixed effect hypothesis (left panel) is supposedly supported by non-linearity (output iv).  AIC and F-test judgements of non-linearity do not provide any evidence for or against the mixed effect hypothesis, as all points are in the "not worth mentioning" range.  The results of the Kolmogorov-Smirnov test are more interesting.  Non-linearity as judged by this test provides infinite support for the mixed effects hypothesis against zero effects (the orange point at the top margin).  It also provides strong support for the mixed effects hypothesis compared to the hypothesis of a weak effect ($\delta = 0.2$).  However, non-linearity provides substantial or strong support *against* the mixed effects hypothesis compared to every other hypothesis examined.  
